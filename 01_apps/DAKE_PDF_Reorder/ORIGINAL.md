...
PDFのページサムネイルをドラッグ＆ドロップで並べ替え、別名PDFとして保存できるWindows向けアプリです。

## 対象ユーザー

- Windowsで小さな実務作業を短く終わらせたい人
- DakePDFページ並べ替え の用途に合う単機能ツールを探している人
...
- 概要: PDFのページサムネイルをドラッグ＆ドロップで並べ替え、別名PDFとして保存できるWindows向けアプリです。
- 使い方: 既存READMEを暫定参照
- 必要なもの: Windows環境
...
- 作品ファイル: booth_ready/DakePDF_Reorder.zip
- GitHub Release: https://github.com/yukiPHZ/dake-series/releases/tag/DAKE_PDF_Reorder_v1.0.0
- BOOTH URL: https://peakheadz.booth.pm/items/8448206

## Store表示用情報

- 商品名: DakePDFページ並べ替え
- キャッチ: PDFのページサムネイルをドラッグ＆ドロップで並べ替え、別名PDFとして保存できるWindows向けアプリです。
- 説明: PDFページ順をドラッグで並べ替えます。
...
- GitHub Release: https://github.com/yukiPHZ/dake-series/releases/tag/DAKE_PDF_Reorder_v1.0.0
- BOOTH: https://peakheadz.booth.pm/items/8448206
...
- ページ順の正本は内部のページ配列とし、画面順および保存後PDFのページ順を一致させる。
- ドラッグ中は確定順を変更せず、挿入位置markerだけを更新する。ページ順の確定はドロップ時に1回だけ行う。
- ページカードのCanvas itemは保持して再利用し、ドラッグやリサイズなどの軽い操作で全削除・全再生成しない。
- ドロップ後は移動の影響範囲だけを再配置し、順序番号を更新する。
- 生成済みのページthumbnailはドラッグ・並び替え・リサイズで再生成しない。
