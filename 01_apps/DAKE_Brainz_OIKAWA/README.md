...
OIKAWAは、補助脳BRAINZへ再接続するための小さな端末です。

OIKAWAを開くことは、単なるアプリ起動ではなく、補助脳への再接続要求です。熱がある時には不要で、熱が冷めた時、止まった時、何をしていたか分からなくなった時に、BRAINZへ戻る入口になります。
...
QPCS（Quiet Personal Cognitive System）では、OIKAWAを検索・原本表示・熾火・熱提案・通知表示の前面UIとして育てます。Phase 1では完全統合せず、BRAINZが書き出す `data/config/qpsc_brainz_status.json` を読み、小さな通知/提案エリアに起床状態を表示します。
...
OIKAWAはQPCS v0.1で、BRAINZに保存された記憶を探して、戻って、読む前面UIとして固定します。
...
OIKAWAはv0.2で、QPCSの前面UIとして「探して、戻って、読む」場所から、必要な記憶が静かに側へ戻ってくる場所へ進みました。判断代行はせず、原本へ戻れる入口を置きます。

- QPCS状態確認でBRAINZ / OIKAWA連携を軽く確認できます
- 原本プレビューを中央に置き、Markdown / TXTをそのまま読めます
- 通知、熾火、ORBITから原本へ戻れます
- Ollama熱補助は熾火候補に短い気配を添えるだけです
- 巡回 / 再訪で、何度も戻っている記憶を扱います
- 側に在る表示で、通知ではない静かな浮上を扱います
- 黒基調と日本語可読性を優先します
...
## QPCS Phase 2: BRAINZ取り込み通知

Phase 2では、BRAINZが保存する `data/config/qpsc_notifications.json` を読み、OIKAWAのQPCS通知欄に未読通知を最大3件まで表示します。
...
- 右カラムはQPCS通知、熾火、今日の整理 / ORBIT、次の候補をサブ情報としてまとめます
- フッターはQPCS表記に変更し、BRAINZにある記憶をOIKAWAが呼び戻す文脈を表示します
...
## QPCS Patch: OIKAWAウインドウ外観をBRAINZ寄せ
...
## QPCS Phase 8: BRAINZからOIKAWAへ

Phase 8では、BRAINZ側にOIKAWAを開く導線を追加しました。BRAINZは取り込み母艦として状態と設定に集中し、検索・原本表示・熾火・ORBITはOIKAWA側で受け持ちます。

## QPCS Phase 10: QPCS状態確認

Phase 10では、OIKAWAに小さなQPCS状態確認を追加しました。実使用前にBRAINZ / OIKAWA連携の現在地を軽く確認できます。
...
## QPCS Phase 11: ChatGPT export取り込み後
...
QPCSのOIKAWA / BRAINZは黒基調の静かなUIを基本にします。
...
- Ollama未起動でもQPCSは動き、熱補助は眠っている状態として表示します
...
Phase 16では、通知ではなく、静かに浮いてくる記憶として「側に在る」表示を追加しました。

- 「側に在る」はポップアップせず、音も鳴らさず、右カラムに小さく表示します
- 候補は巡回ログ、熾火候補、未読通知、Ollama熱補助を弱く組み合わせて生成します
- 表示文は命令ではなく、最近また戻っている、深夜に巡回されている、熱の気配がある、という状態として扱います
- 原本主義を維持し、候補から中央の原本プレビューへ戻れます
- ORBITには側に在る記憶数、深夜巡回数、熱補助付き巡回数を反映します


## QPCS Phase 18: Slack静かな通知導線

Phase 18では、「側に在る」や巡回、熾火、熱補助の候補から、Slackへ小さく置く通知導線を追加しました。

- Slack通知はポップアップ思想ではなく、外部に少し置く再接続として扱います
- 送信対象は側に在る、巡回、熾火、Ollama熱補助ありの候補に限定します
- 1日最大件数と重複防止履歴で大量通知を避けます
- 送信履歴は `data/config/qpsc_slack_notify_history.json` に保存しますが、Git管理しません
- Webhook失敗でもOIKAWAは落ちず、内部循環を優先します

## QPCS Phase 19: 時間を跨いだ熱

Phase 19では、最近の熱だけでなく、しばらく静かだった後にまた戻ってくる記憶を扱います。

- 「静かだった記憶」は、7日以上空いた再訪、深夜巡回率、最近の巡回増加、熱補助を弱く組み合わせます
- 表示はおすすめではなく、時間を跨いで残っている状態として扱います
- ORBITには静かだった記憶数、深夜巡回率、時間を跨いだ再訪数を反映します
...
## QPCS v0.3: OIKAWA / OpenClaw / Obsidian
...
- published noteはBRAINZ側で正本Markdownとして保存されます
- OIKAWAでは通知、熾火、熱検索、巡回、側に在る候補から原本へ戻れます
- note専用UIはまだ作り込まず、既存の原本主義の流れに沿わせます
...
```json
{
  ...
  "update_summary": "QPCS v0.1として検索・原本表示・通知・熾火・ORBITの前面UIを正本化しました。",
...
## QPCS Phase 22: BRAINZ通常運用UIとの分担
...
- OIKAWAは全文プレビューをさらに控え、選択中の正本と `原本を開く` を優先する。
- 記憶ルート表示は短くし、PEAKHEADZ_ROOTを通常ルートとして扱う。
- 熾火、ORBIT、巡回、側に在る、静かだった記憶を中心に、情報量を減らした巡回端末へ寄せる。

## OIKAWA最小巡回UI化

- OIKAWAは全文プレビューアプリではなく、正本ニュース端末として扱う。
- 中央は正本ニュースと短い今日の整理に絞り、数字の羅列や重複表示を前面から外す。
- 空の関連カードや空の選択中カードは出さず、選択時だけ `原本を開く` を表示する。
- 熾火、巡回、側に在る、静かだった記憶を、戻るきっかけとして静かに置く。

## QPCS Patch: OIKAWAフッター最小化 / 空説明削減

- フッターは `QPCS — Quiet Personal Cognitive System by Yukihiko Kikuta` のみを表示します。
- OIKAWAは思想を語るUIではなく、記憶が浮く端末として扱います。
- 側に在る / 巡回 / 静かだった記憶 / 熱補助は、候補がある時だけ静かに表示します。
- 空状態は大きく説明せず、正本ニュースと熾火を中心にします。

## QPCS Patch: OIKAWA宇宙感強化 / カード透過風

- Tkinterの完全透過ではなく、暗い薄塗りと弱い枠線で透過風に寄せます。
...
## QPCS v0.4: OIKAWAは再接続要求

OIKAWAは常駐OSではありません。熱い時に使う道具でもありません。

OIKAWAは「戻る場所」です。さらに言えば、補助脳BRAINZへ接続するための生体アクセスキーです。

```text
OIKAWAを開く = 再接続要求
```

熱がある時、菊田は前へ進みます。ライブ配信、Codex、実装、音楽、動画、記事、実務。その時、OIKAWAはいりません。すでに流れているからです。

熱が冷めた時、止まった時、忘れた時、どこへ向かっていたか分からなくなった時に、OIKAWAを開きます。OIKAWAは「おかえり。線はまだ続いています。」を返す場所です。
...
OIKAWAは命令しません。大量の数字、TODO、進捗表を主役にしません。必要なのは少ない情報です。
...
OIKAWAは、忘れてもいいが戻れるようにしておくための端末です。
