...
class PDFMarkerApp:
    ...
    def _save_worker(self, input_path: Path, output_path: Path, markers: list[Marker]) -> None:
        ...
        try:
            with fitz.open(str(input_path)) as output_doc:
    ...
    def _event_to_page_point(self, event: tk.Event, require_inside: bool) -> tuple[float, float] | None:
        ...
        return (
            page_rect.x0 + local_x / self.render_scale,
            page_rect.y0 + local_y / self.render_scale,
        )
    ...
    def _update_page_label(self) -> None:
        if self.doc is None:
            ...
            return
        page_count = getattr(self.doc, "page_count", 0)
        self.page_var.set(UI_TEXT["status_page"].format(current=self.page_index + 1, total=page_count))
    ...
    def _update_controls(self) -> None:
        ...
        self.reset_button.configure(state=tk.NORMAL if self.markers and not self.busy else tk.DISABLED)
