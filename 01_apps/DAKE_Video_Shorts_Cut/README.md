...
- MP4を1本選ぶ
- ffprobeで動画情報を確認
- 動画全体から開始位置を分散して計画し、ショート候補を3本作成
...
- 候補ごとの切り出し開始時刻を結果欄・レビュー・QR転送ページに表示
...
- ウインドウアイコンにDAKE共通アイコンをより確実に反映するよう補正しました。
- 切り出し開始位置の簡易探索を追加しました。
- 単純3分割より、候補が散るように開始位置を調整します。
- 長尺動画では冒頭すぎる部分と終了ギリギリを避けます。
- 候補行、おすすめカード、`shorts_review.txt`、`shorts_review.json`、QR転送ページに開始時刻を表示します。
- ただし、v0.8ではAIで開始位置を決める段階ではありません。
- 今後、文字起こし/Ollama/OpenAIで開始位置自体を選ぶ予定です。
