...
UI_TEXT = {
    ...
    "main_heading": "工程カレンダー",
    "main_subtitle": "現場名・期間・工程を入力して、A4横の工程カレンダーPDFを作成します。",
    "section_site": "現場情報",
    ...
    "status_processing": "PDFを作成しています...",
    "status_saved": "PDFを保存しました。",
    ...
    for year in years:
        ...
        while current <= date(year, 12, 30):
            if current not in year_holidays:
                if current - timedelta(days=1) in year_holidays and current + timedelta(days=1) in year_holidays:
...
def generate_pdf(request: PdfRequest) -> Path:
    ...
    period_text = f"{format_date_jp(request.start_date)}〜{format_date_jp(request.end_date)}"
    ...
    for page_index, chunk in enumerate(chunks, start=1):
        ...
        pdf.setFillColor(color_hex(PDF_COLORS["text"]))
        pdf.drawString(margin_x, info_y, f"{UI_TEXT['pdf_target_period']}：{period_text}")
        pdf.drawString(margin_x + 270, info_y, f"{UI_TEXT['pdf_target_month']}：{month_text}")
        pdf.drawRightString(page_width - margin_x, info_y, f"{UI_TEXT['pdf_staff']}：{request.staff_name.strip()}")
        ...
        pdf.drawString(
            margin_x,
            info_y - 16,
            f"{UI_TEXT['pdf_page_range']}：{format_date_jp(range_start)}〜{format_date_jp(range_end)}",
        )
        ...
        if request.completion_date is not None:
            ...
            draw_wrapped_text(pdf, comment_text, margin_x, comment_y, 500, serif_font, 10.2, 13)
        ...
        pdf.setFillColor(color_hex(PDF_COLORS["muted"]))
        pdf.drawString(margin_x, 39, UI_TEXT["footer_series"])
        pdf.showPage()
...
class WorkCalendarApp:
    ...
    def build_input_sections(self) -> None:
        ...
        save_entry.grid(row=2, column=1, columnspan=3, sticky="ew", pady=6)
        self.make_button(period_panel, UI_TEXT["button_browse"], self.choose_save_folder, compact=True).grid(
            row=2,
            column=4,
            sticky="e",
            padx=(10, 0),
    ...
    def build_actions(self) -> None:
        ...
        self.create_button = self.make_button(action_panel, UI_TEXT["button_create_pdf"], self.on_create_pdf)
    ...
    def build_footer(self) -> None:
        ...
        self.link_label(right, UI_TEXT["footer_estimate"], LINK_TARGETS["estimate"]).pack(side="left")
        tk.Label(
            right,
        ...
        self.link_label(right, UI_TEXT["footer_instagram"], LINK_TARGETS["instagram"]).pack(side="left")
        tk.Label(
            right,
            text=UI_TEXT["footer_separator"] + UI_TEXT["footer_copyright"],
            font=self.fonts["footer"],
            fg=COLORS["footer"],
            bg=COLORS["background"],
        ).pack(side="left")
    ...
    def collect_request(self) -> PdfRequest | None:
        ...
        for row in self.event_rows:
            ...
            if not content:
                default_label = next(
                    str(item["label"]) for item in UI_TEXT["event_items"] if item["key"] == key
                )
