...
このファイルは `DAKE_PDF_Merge_Mini` の真の正本です。

README.md、DAKE_META、release_body.md、booth_product.txt、Store表示などは、このファイルから派生するビューです。

## 基本情報

- app_id: `dake_pdf_merge_mini`
- title: DakePDF結合mini
- short_title: PDF結合mini
- category: PDF
- status: available
...
PDFを最大5ファイルまで選択し、表示順に1つのPDFへ結合できるWindows向けminiアプリです。

## 対象ユーザー

- Windowsで小さな実務作業を短く終わらせたい人
- DakePDF結合mini の用途に合う単機能ツールを探している人

## 解決する困りごと

最大5ファイルまでのPDFを並べ替えて結合します。
...
- 概要: PDFを最大5ファイルまで選択し、表示順に1つのPDFへ結合できるWindows向けminiアプリです。
- 使い方: 既存READMEを暫定参照
- 必要なもの: Windows環境
...
- 商品名: DakePDF結合mini
- 価格案: 500円
- 商品紹介文: 最大5ファイルまでのPDFを並べ替えて結合します。
...
- 作品ファイル: booth_ready/DakePDF_Merge_Mini.zip
- GitHub Release: https://github.com/yukiPHZ/dake-series/releases/tag/DAKE_PDF_Merge_Mini_v1.0.0
- BOOTH URL: https://peakheadz.booth.pm/items/8448200

## Store表示用情報

- 商品名: DakePDF結合mini
- キャッチ: PDFを最大5ファイルまで選択し、表示順に1つのPDFへ結合できるWindows向けminiアプリです。
- 説明: 最大5ファイルまでのPDFを並べ替えて結合します。
...
- GitHub Release: https://github.com/yukiPHZ/dake-series/releases/tag/DAKE_PDF_Merge_Mini_v1.0.0
- BOOTH: https://peakheadz.booth.pm/items/8448200
...
- mini版として、操作可能なウインドウを早く表示する。PDF処理ライブラリはPDF追加・結合時に読み込み、GUI表示前の初期化へ含めない。
- `items` の順序を画面順・PDF結合順の正本とする。
- PDF追加では新しいカードだけを生成し、並び替えでは既存カードの再配置だけを行う。
- 削除では対象カードだけを破棄し、残るカードとサムネイルは再利用する。
- 並び替え・削除・ウインドウサイズ変更で、PDFやサムネイルを再読込・再生成しない。
- PDF読込と結合はUI threadから分離し、処理中もウインドウの応答を止めない。
