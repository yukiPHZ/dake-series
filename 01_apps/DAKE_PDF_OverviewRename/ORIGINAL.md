...
- `特大` は内容確認を優先した表示とし、低解像度の「大」画像を単純拡大しません。thumbnail base imageを特大表示に耐える最大解像度で1回生成し、小・標準・大・特大はそこから縮小します。
...
- 初期表示はプレビューウインドウへ収まるfit表示を100%とします。
- プレビュー上のマウスホイールで50%から300%まで段階的に拡大・縮小し、現在倍率を表示します。イベントはプレビュー内で消費し、本体一覧をスクロールしません。
- 拡大縮小は可能な限りマウスカーソル位置を基準にし、拡大後は左ドラッグでパンできます。
- fit表示より大きい表示でも文字や線を実用的に確認できる高解像度base imageをworkerで生成します。base解像度が不足するときだけdebounceしたLatest Jobで再renderし、ホイール連打で未開始jobを蓄積しません。
- プレビューを開いたまま別カードへ切り替えた場合と、閉じて開き直した場合はzoomとpanをfitへ戻し、古いPDFまたは古いgenerationの結果を適用しません。
- fit状態のwindow resizeは新しい表示領域へ追従し、明示的なzoom中は倍率を不用意にresetしません。
- 外部PDFビューアを主経路にしません。
