...
- 画像一覧は画像ごとの既存Canvas itemを再利用し、追加では新しい行だけを生成する
- 選択変更では変更前後の行だけを更新し、上下移動では対象2行だけを再配置する
- 削除では対象行を破棄して後続行だけを詰め、軽い操作で一覧全体をdestroy / recreateしない
- ウインドウリサイズでは既存Canvas itemの座標と幅だけを更新し、サムネイルを再生成しない
- サムネイルと画像寸法は追加時に1回取得して画像項目に保持し、選択・移動・削除では画像ファイルを再読込しない
- PDF出力順は画面一覧のデータ配列順と常に一致させる
